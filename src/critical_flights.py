...
def enhanced_critical_flights_analysis(df):
    """Enhanced critical flights analysis with multiple centrality measures"""
    
    print("--- Enhanced Critical Flights Analysis Started ---")
    
    G = nx.DiGraph()
    
    # Build more sophisticated graph
    flight_nodes = {}
    ...
    for idx, row in df.iterrows():
        ...
        # Add flight node with attributes
        ...
        # Connect flights using same aircraft (aircraft routing chains)
        ...
        for _, next_flight in same_aircraft_flights.iterrows():
            ...
            if turnaround_time > 0:
                weight = 1 / max(turnaround_time, 0.5)  # Inverse relationship
                G.add_edge(flight_id, next_flight_id, 
                          weight=weight,
                          connection_type='aircraft_routing',
                          turnaround_hours=turnaround_time)
        
        # Connect flights with passenger connections (hub connections)
        if 'Mumbai (BOM)' in row['From']:  # Hub airport
            ...
            for _, conn_flight in connecting_flights.iterrows():
                ...
                if conn_flight_id != flight_id:
                    G.add_edge(flight_id, conn_flight_id, 
                              weight=0.4,  # Passenger connection weight
    ...
    # Create enhanced critical flights dataframe
    ...
    if len(critical_df) > 0:
        # For backward compatibility, also create the simple version
...
def find_critical_flights():
    """
    Identifies critical flights using enhanced network analysis.
    """
    ...
    # Calculate departure delays for context
